...
打包：pyinstaller --onefile --windowed gitee_updater.py
"""
...
CONFIG_FILE = "gitee_updater_config.json"
...
def build_remote_url(user: str, repo: str, token: str) -> str:
    """构造带 token 的 HTTPS 远程地址（适配 Gitee）。"""
    repo = repo.strip().rstrip("/")
    if not repo.endswith(".git"):
        repo += ".git"
    return f"https://{user.strip()}:{token.strip()}@gitee.com/{user.strip()}/{repo}"


def build_clean_url(user: str, repo: str) -> str:
    """不含 token 的干净远程地址（适配 Gitee）。"""
    repo = repo.strip().rstrip("/")
    if not repo.endswith(".git"):
        repo += ".git"
    return f"https://gitee.com/{user.strip()}/{repo}"
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # ── 账号配置 ──
        acct_frame = ttk.LabelFrame(self, text="Gitee 账号配置")
        acct_frame.pack(fill="x", **pad)

        # 用户名
        ttk.Label(acct_frame, text="Gitee 用户名/机构名：").grid(row=0, column=0, sticky="w", **pad)
        self.gitee_user = tk.StringVar()
        ttk.Entry(acct_frame, textvariable=self.gitee_user, width=30).grid(
        ...
        # Token
        ttk.Label(acct_frame, text="私人令牌 (Personal Access Token)：").grid(row=2, column=0, sticky="w", **pad)
        ...
        ttk.Checkbutton(misc_frame, text="记住配置（保存到 gitee_updater_config.json）",
    ...
    def _maybe_save_config(self):
        if self.remember.get():
            save_config({
                "gitee_user":       self.gitee_user.get(),
    ...
    def _validate(self, need_local=True) -> bool:
        if not self.gitee_user.get().strip():
            messagebox.showwarning("提示", "请填写 Gitee 用户名或机构名。")
            return False
        if not self.repo_name.get().strip():
            messagebox.showwarning("提示", "请填写仓库名（Repo）。")
            return False
        if not self.token.get().strip():
            messagebox.showwarning("提示", "请填写 Gitee 私人令牌 (Token)。")
            return False
    ...
    def _push_task(self):
        ...
        user   = self.gitee_user.get().strip()
        ...
        if code == 0:
            self._log("✅ Push 成功！")
            self._log("⚠️  建议在 Gitee 设置中定期轮换（修改）此私人令牌。")
